...
freeze_cell_payload='{"cell_code":"QA-CELL-534","name":"Freeze precheck cell","layout_geojson":{"type":"FeatureCollection","features":[]},"robot_model":"QA-Robot-Y","controller_model":"QA-Control-Y","max_reach_mm":1800,"owner_team":"QA Integration"}'
request "engineer creates freeze precheck cell" 201 POST "/cells" "$engineer_token" "$freeze_cell_payload"
freeze_cell_id="$(jq -r '.data.id' <<<"$last_body")"
request "freeze blocked without zones or programs" 409 POST "/cells/$freeze_cell_id/freeze" "$engineer_token"
require_json '.error.code == "freeze_precheck_failed" and (.error.details.issues | length) == 2' "empty cell lists both issue categories"
require_json '.data == null' "failed precheck returns no cell payload"
freeze_zone_payload="$(jq -nc --argjson cell "$freeze_cell_id" '{robot_cell_id:$cell,name:"QA freeze gate",zone_type:"restricted",polygon_geojson:{type:"Polygon",coordinates:[[[600,-300],[1200,-300],[1200,300],[600,300],[600,-300]]]},min_height_mm:0,max_height_mm:2100,speed_limit_mm_s:120,access_rule:"Gate lock must precede motion"}')"
request "engineer adds draft zone to precheck cell" 201 POST "/zones" "$engineer_token" "$freeze_zone_payload"
freeze_zone_id="$(jq -r '.data.id' <<<"$last_body")"; freeze_zone_version="$(jq -r '.data.version' <<<"$last_body")"
request "freeze blocked by inactive zone" 409 POST "/cells/$freeze_cell_id/freeze" "$engineer_token"
require_json '.error.code == "freeze_precheck_failed" and (.error.details.issues | length) == 2 and ([.error.details.issues[] | contains("QA freeze gate")] | any)' "inactive zone listed by name"
request "activate precheck zone" 200 POST "/zones/$freeze_zone_id/activate" "$engineer_token" "$(jq -nc --argjson version "$freeze_zone_version" '{version:$version}')"
request "freeze blocked without ready program" 409 POST "/cells/$freeze_cell_id/freeze" "$engineer_token"
require_json '.error.code == "freeze_precheck_failed" and (.error.details.issues | length) == 1' "only missing program issue remains"
freeze_program_payload="$(jq -nc --argjson cell "$freeze_cell_id" '{robot_cell_id:$cell,program_code:"QA-FREEZE-533",version:1,trajectory:[{x_mm:0,y_mm:0,z_mm:700,time_ms:0,speed_mm_s:300},{x_mm:900,y_mm:100,z_mm:760,time_ms:2800,speed_mm_s:300}],tool_radius_mm:120,payload_radius_mm:90,interlock_sequence:[{name:"gate_locked",sequence:1,depends_on:[]}]}')"
request "programmer imports precheck program" 201 POST "/programs" "$programmer_token" "$freeze_program_payload"
freeze_program_id="$(jq -r '.data.id' <<<"$last_body")"
request "freeze still blocked for uploaded program" 409 POST "/cells/$freeze_cell_id/freeze" "$engineer_token"
request "parse precheck program" 200 POST "/programs/$freeze_program_id/transition" "$programmer_token" '{"target_state":"parsed"}'
request "freeze still blocked for parsed program" 409 POST "/cells/$freeze_cell_id/freeze" "$engineer_token"
request "ready precheck program" 200 POST "/programs/$freeze_program_id/transition" "$programmer_token" '{"target_state":"ready"}'
request "freeze passes once issues resolved" 200 POST "/cells/$freeze_cell_id/freeze" "$engineer_token"
require_json '.data.cell_state == "frozen"' "cell frozen after precheck passes"
request "repeat freeze reports state conflict" 409 POST "/cells/$freeze_cell_id/freeze" "$engineer_token"
require_json '.error.code == "state_conflict"' "repeat freeze state conflict"
