...
@Component({
  ...
  template: `
    ...
    @if (store.freezeIssues().length) {
      <section class="freeze-issues">
        <header><lucide-icon name="shield-check" [size]="15" /><strong>Freeze pre-check blocked this release. Resolve each item, then retry.</strong></header>
        <ul>@for (issue of store.freezeIssues(); track issue) { <li>{{ issue }}</li> }</ul>
      </section>
    }
